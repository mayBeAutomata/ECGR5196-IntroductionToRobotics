 ...
 * Modified by Somto Anyaegbu & Ajay Chundi
 *
...
void loop() {
  ...
  static bool done = false; //boolean variable to determine when motors stop
  ...
  //Count number of sensors reading value to determine
  //end of line. 
  //Line present if sensor value > 750 
  int sensorCount = 0;
  ...
  //If the robot derails off the line 
  //Or reaches end, motors are disabled
  //Minimum number of sensor < 5
   if (sensorCount >= 5 || linePos == 0) {
    Serial.println("End");
    done = true;
    return;
   }
